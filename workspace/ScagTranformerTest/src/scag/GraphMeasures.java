...
class Triangulation {
    ...
    public double[] compute(double[] dataX, double[] dataY, boolean quick) {
    	px = new int[dataX.length];
    	py = new int[dataY.length];
    	for (int p = 0; p < dataX.length; p++) {
    		px[p] = (int) (dataX[p]*1000);
    		py[p] = (int) (dataY[p]*1000);
    	}
        if (px.length < 2)
            return null;
    ...
    private double computeCutoff(double[] lengths) {
        ...
        int n95 = lengths.length *19 / 20;
        int n10 = lengths.length *2  / 20;
        return lengths[n95] +  0.75*(lengths[n95] - lengths[n10]);
    }
    ...
    private double computeStriationMeasure() {
        ...
        return numEdges*1.3f / (double) mstEdges.size();
    }
