...
public class ScagnosticsVIS extends PApplet {
	public static String filename = "data2/VIS_papers.txt";
	public static int nC = 500; // Unemployment rate
	...
	public static int nY = 60; // No data for 2016 and 2017
	...
	public static void main(String args[]){
	  PApplet.main(new String[] { ScagnosticsVIS.class.getName() });
    }
	...
	public void setup() {
		...
		for (int c = 0; c < nC; c++) {
			for (int y = 0; y < nY; y++) {
			//	System.out.println(y+"\t"+countries[c]+"\t"+data[1][y][c]+"\t"+dataS[1][y][c]);	
			}
		}	
	...
	private static void getData() {
		ScagnosticsVIS w = new ScagnosticsVIS();
	...
	private static void normalizeMinMax() {
		...
		for (int v = 0; v < nV; v++) {
			for (int y = 0; y < nY; y++) {
				for (int c = 0; c < nC; c++) {
					if (dataMin[v][y]< mins[v])
						mins[v] = dataMin[v][y];
					if ( dataMax[v][y]>maxs[v])
						maxs[v]=dataMax[v][y] ;
				}
			}
			System.out.println("variable "+v+" min="+mins[v]);
			System.out.println("variable "+v+" max="+maxs[v]);
		}
