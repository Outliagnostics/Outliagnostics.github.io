...
class Triangulation {
	...
	public final static String[] scagnosticsLabels = { "Outlying", "Skewed", "Clumpy", "Sparse", "Striated", "Convex",
	...
	private double computeOutlierMeasure() {
		// Tuan modify to take care the problem when we have too few bins -> high Outlying;
		if (bdata.binCounts<10){
			//ratio = 1-bdata.binCounts/10.;
			return 0.9;
		}	
		else
	...
	private double computeStringyMeasure() {
		...
		for (int i=0;i<px.length;i++){
			int x = px[i];
			int y = py[i];
			countX+= (x-RESOLUTION/2);
			countY+= (y-RESOLUTION/2);
		}
		double totalAsymmetric = px.length*RESOLUTION/2;
		double assymetricX = Math.abs(countX)/totalAsymmetric;
		double assymetricY = Math.abs(countY)/totalAsymmetric;
		double stringy = result * result * result * result;
		//	System.out.println("	stringy:"+stringy+"	assymetricX:"+assymetricX+" assymetricY:"+assymetricY);
		
		double thred =0.3;
		if (assymetricX>thred) 
			stringy *= (1-assymetricX);
		if (assymetricY>thred) 
			stringy *= (1-assymetricY);
		return stringy;
	}
	...
	private double computeMonotonicityMeasure() {
		...
		// Tuan modify to take care the problem when we have too few bins -> high Monotonic;
		//System.out.println("Total count = "+totalCount+" binCount = "+bdata.binCounts);
		double ratio =1;
		if (bdata.binCounts<100)
			ratio = bdata.binCounts/100.;
		return s * s * ratio;
	}
	...
	private double computeSparsenessMeasure() {
		double al = alphaArea/(BinnedData.RESOLUTION*BinnedData.RESOLUTION);
	/*	if (al>0.5)
			System.out.println("Alpha shape:"+al);
		int n = sortedOriginalMSTLengths.length;
		if (n == 0)
			return Double.NaN;
		int n90 = (9 * n) / 10;
	...
	private double computeStriationMeasure() {
		...
		double assymetricX = Math.abs(countX)/totalAsymmetric;
		double assymetricY = Math.abs(countY)/totalAsymmetric;
		double striated = numEdges / (double) mstEdges.size();
		//if (main.Main.print)
		//	System.out.println("	striated:"+striated+"	assymetricX:"+assymetricX+" assymetricY:"+assymetricY);
		
		double thred =0.7;
		if (assymetricX>thred) 
			return striated*(1+0.2-assymetricX);
		else if (assymetricY>thred) 
			return striated*(1+0.2-assymetricY);
		else 
		//double ratio =1;
		//if (bdata.binCounts<800)
		//	ratio = bdata.binCounts/800.;
		return striated;
	}
	...
	private void swapTest(Edge e11) {
		try{
            ...
            System.err.println("ERROR: StackOverflowError");
            return;
        }
	}
